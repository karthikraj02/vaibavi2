    ...
    <nav className="fixed w-full glass-nav z-50 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6">
          <Link to="/search" className="text-gray-300 hover:text-neonCyan font-medium transition-colors duration-300">
            Search Flights
          </Link>
          <Link to="/tracking" className="text-gray-300 hover:text-neonCyan font-medium transition-colors duration-300">
            Live Tracking
          </Link>
          {user ? (
            <>
              <Link to="/dashboard" className="text-gray-300 hover:text-neonCyan font-medium transition-colors duration-300">Dashboard</Link>
              <Link to="/tickets" className="text-gray-300 hover:text-neonCyan font-medium transition-colors duration-300">My Tickets</Link>
              {user.role === 'admin' && (
                <Link to="/admin" className="text-neonPurple hover:text-white font-bold transition-colors duration-300">Admin</Link>
              )}
...
const Home = () => (
  <div className="min-h-screen flex flex-col items-center justify-center text-center px-4 pt-20 relative overflow-hidden">
    ...
    <div className="relative z-10 glass-panel p-12 md:p-20 rounded-3xl max-w-4xl border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
      ...
      <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed font-light">
        Book flights with secure Stripe payments, live Duffel offers, and instant e-ticket confirmation.
      </p>
      <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
        <Link to="/search" className="neon-button bg-gradient-to-r from-neonCyan to-blue-600 px-8 py-4 rounded-full text-lg font-bold">
          Initialize Search
        </Link>
        <Link to="/register" className="px-8 py-4 rounded-full text-lg font-bold border border-white/20 text-white hover:bg-white/5 transition-all">
          Create Access
        </Link>
      </div>
    </div>
      ...
      <div className="relative min-h-screen text-gray-100 flex flex-col">
        ...
        <main className="flex-grow flex flex-col pt-20">
          <Routes>
            ...
            <Route path="/refunds" element={<ProtectedRoute><RefundTracker /></ProtectedRoute>} />
